...
namespace BankOfHogwarts.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("login")] // Specific route for login
        public IActionResult Authenticate([FromBody] CustomerLoginDto loginDto)
        {
            IActionResult response = Unauthorized();
            var token = string.Empty;

            // Using the UserType from DTO
            string userType = loginDto.UserType.ToLower();
            ...
            if (userType == "customer")
            {
                ...
                {
                    token = GenerateJwtToken(customer.Email, customer.FirstName, "Customer");
                    return Ok(new { Token = token ,CustomerId = customer.CustomerId});  //
                }
            }
